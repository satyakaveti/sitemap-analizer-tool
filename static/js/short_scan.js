...
form.addEventListener('submit', async (e) => {
    ...
    startBtn.textContent = 'Starting Short Scan...';

    try {
        const resp = await fetch('/api/scan', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({sitemaps, concurrency, scan_type: 'SHORT'}),
        });
        const data = await resp.json();

        if (!resp.ok) {
            throw new Error(data.detail || 'Failed to start short scan');
        }
        ...
        startBtn.textContent = 'Start Short Scan';
    }
});
...
function showResults(data) {
    ...
    const resultSeo = document.getElementById('result-seo');
    if (resultSeo) resultSeo.textContent = data.seo_issues.toLocaleString();
    ...
    if (issuesLink) issuesLink.href = `/scan/${currentScanId}/issues`;
...
function renderSummaryCards(data) {
    ...
    if (data.content_summary && data.content_summary.length) {
        html += '<div class="summary-group"><h3>Content Size Issues</h3><table class="summary-table">';
